...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Converse360 account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Converse360 account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our Converse360 account';
    const message = `Join ${accountName} on Converse360 using this link (valid for ${result?.expiresInDays} days): ${url}`;
        ...
        {result ? (
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-accent-amber">
              ...
              {/* Anchor styled with `buttonVariants` rather than wrapping
                  in <Button asChild>. The Converse360 Button is the Base UI
