...
export function TemplateGallery() {
  ...
  const matches = useMemo(() => {
    ...
    return TEMPLATE_SLUGS.map((slug) => AUTOMATION_TEMPLATES[slug]).filter((t) => {
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 self-start rounded-lg border border-primary/30 bg-primary/10 px-3 py-2 text-xs font-medium text-primary transition-colors hover:bg-primary/15 sm:self-auto"
        >
        ...
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {matches.map((template) => (
            <TemplateCard
              ...
              template={template}
              snapshot={snapshot}
              onUse={() => router.push(`/automations/new?template=${template.slug}`)}
            />
          ))}
        </ul>
      )}
...
function TemplateCard({
  ...
  const requirements = useTemplateReadiness(
    template,
    snapshot,
  ...
  return (
    <li>
      <button
        ...
        onClick={onUse}
        className={cn(
          "flex h-full w-full flex-col items-start rounded-xl border border-border bg-card p-4 text-left transition-colors",
      ...
      >
        ...
        <span className="mt-auto flex w-full items-center gap-2 pt-3 text-[11px] text-muted-foreground">
