...
//
// The reachability cases matter most: a linear node type missing from
// `outgoingEdges` looks TERMINAL to the BFS, so everything downstream
// is reported "unreachable" — an error that points at innocent nodes
// while the real cause is one missing `case`.
// ============================================================
...
describe('validateFlowForActivation — new node types', () => {
  ...
  it('treats every linear node type as reachable-through', () => {
    ...
    for (const [node_type, config] of linear) {
      ...
      // No "unreachable" and no "unknown node type" — the two failures a
      // missing case produces.
      expect(
  ...
  it('does not warn about the window for a wait under 24 hours', () => {
    const issues = flowWith([
      {
        node_key: 'n',
        node_type: 'wait',
        config: { duration: 23, unit: 'hours', next_node_key: 'done' },
      },
    ]);
    expect(issues).toEqual([]);
  });

  it('requires body text on a product LIST (Meta rejects the send without it)', () => {
    const issues = flowWith([
      {
        node_key: 'n',
        node_type: 'send_products',
        config: {
          mode: 'list',
          product_retailer_ids: ['a', 'b'],
          next_node_key: 'done',
        },
      },
    ]);
    expect(
      issues.some((i) => i.severity === 'error' && i.field === 'body_text')
    ).toBe(true);
  });
  ...
  it('treats start_flow and ai_handoff as terminal', () => {
    const issues = validateFlowForActivation(
      { ...validFlow, entry_node_id: 's' },
      [
        { node_key: 's', node_type: 'start', config: { next_node_key: 'n' } },
        {
          ...
          config: { flow_id: '11111111-1111-1111-1111-111111111111' },
        },
      ]
    );
  ...
  it('requires a flow on start_flow', () => {
    const issues = validateFlowForActivation(
      { ...validFlow, entry_node_id: 's' },
      [
        { node_key: 's', node_type: 'start', config: { next_node_key: 'n' } },
        { node_key: 'n', node_type: 'start_flow', config: {} },
      ]
    );
    expect(
      issues.some((i) => i.severity === 'error' && i.field === 'flow_id')
    ).toBe(true);
  });
});
