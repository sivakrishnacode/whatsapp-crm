  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!autRes.ok) throw new Error(`Failed to load automation (${autRes.status})`)
        if (!logRes.ok) throw new Error(`Failed to load logs (${logRes.status})`)
        const autBody = await autRes.json()
        const logBody = await logRes.json()
        setAutomation(autBody.automation as Automation)
        setLogs((logBody.logs ?? []) as AutomationLog[])
      } catch (err) {
