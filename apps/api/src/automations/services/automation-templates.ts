/**
 * The quick-start template catalogue — API copy.
 *
 * ⚠️ MIRROR OF apps/web/src/lib/automations/templates.ts.
 *   Same slugs, same triggers, same steps, same copy. It is NOT
 *   byte-identical: the two apps have different prettier configs (this
 *   one takes semicolons and single quotes), so a plain `diff` is noisy.
 *   `apps/web/src/lib/automations/template-expand.test.ts` is what
 *   actually holds the web copy honest; the pairing here is maintained
 *   by hand. There is no shared types package yet (same note as
 *   `automation.types.ts`), so change both together.
 *
 * WHAT READS THIS FILE
 *   `POST /automations { template: '<slug>' }` — a caller that wants an
 *   automation seeded server-side without sending the steps. The
 *   dashboard does NOT take that path: its gallery expands the template
 *   in the browser and posts full steps, which is why the web copy is
 *   the one carrying the gallery metadata.
 */
