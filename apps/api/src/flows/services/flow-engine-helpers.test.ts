...
// Ported near-verbatim from apps/web/src/lib/flows/engine.test.ts —
// the pure-helper slice of the old engine module. The interpolateVars
// suite is new (the web original never exported it, so it had no
// direct coverage).
...
describe('interpolateVars', () => {
  it('replaces {{vars.X}} with the stored value', () => {
    expect(interpolateVars('Hi {{vars.name}}!', { name: 'Alice' })).toBe(
      'Hi Alice!',
    );
  });

  it('renders missing / null vars as empty string', () => {
    expect(interpolateVars('Hi {{vars.name}}!', {})).toBe('Hi !');
    expect(interpolateVars('Hi {{vars.name}}!', { name: null })).toBe('Hi !');
  });

  it('stringifies non-string values', () => {
    expect(interpolateVars('n={{vars.n}}', { n: 42 })).toBe('n=42');
    expect(interpolateVars('b={{vars.b}}', { b: true })).toBe('b=true');
  });

  it('handles multiple occurrences and empty templates', () => {
    expect(
      interpolateVars('{{vars.a}}-{{vars.b}}-{{vars.a}}', { a: 'x', b: 'y' }),
    ).toBe('x-y-x');
    expect(interpolateVars('', { a: 'x' })).toBe('');
  });
});
