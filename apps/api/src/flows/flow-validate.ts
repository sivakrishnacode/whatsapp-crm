/**
 * Save-time validation for flows — ported verbatim from
 * apps/web/src/lib/flows/validate.ts.
 *
 ...
 * mid-build. The builder runs the same checks client-side; this
 * server-side copy is authoritative (a broken activate can't slip in
 * via direct API call).
 *
 ...
 *      title. Mirrors the runtime checks inside meta-api.util.ts so
 *      save-time and send-time can never disagree.
 *
 ...
 * `node_key`; trigger-scoped use `scope: 'trigger'`. Field names stay
 * snake_case — they're part of the 422 payload the frontend consumes.
 */

import { INTERACTIVE_LIMITS } from '../whatsapp/meta-api.util';
