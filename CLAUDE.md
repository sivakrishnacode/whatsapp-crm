...
- Selectable plans are **STARTER, GROWTH, ENTERPRISE**. **FREE is retired** (migration 066 set `is_active = false`); it survives only so historical rows can still resolve their `plan_id`. Nothing may offer it again — there is no free tier to downgrade to, so cancellation sets `status = 'cancelled'` and the gate sends the account to `/billing` (or the plan picker, if it somehow still has a trial to spend — see below).
...
- ⚠️ **A LAPSED ACCOUNT GETS `/billing`, NOT THE PLAN PICKER.** `OnboardingState.step` has a fourth value, `billing`, for "the one trial is spent and nothing has been paid" (`!isEntitled && !trialAvailable && hasSubscription`), and `useOnboardingGate` redirects there instead of `/welcome`. Before it, a lapsed workspace was sent to the wizard, where every card advertised a 15-day trial and every button said "Start free trial" — pressing one carried the lapsed window forward (074 refusing a second trial, correctly), landed `expired` again, and re-rendered the same picker. **The button did nothing, for ever**, and `/pricing` was unreachable because it lives inside the gated dashboard. `selectPlan` now throws rather than performing that no-op write. The screen (`app/(onboarding)/billing/page.tsx`) offers only the exits that exist: pay, ask support for more time, or talk to sales — and **no trial badge anywhere**, which is also why it does not reuse `PlanStep`. Teammates get a read-only "ask your owner" variant, because `user_subscriptions` is keyed by `accounts.owner_user_id` and their payment would land on a row nothing reads. `viewer.isOwner` is computed server-side from that column, never from a profile role.
- ⚠️ **`past_due` is ENTITLED.** It is dunning: `get_account_entitlement` grades it `grace`, writes still work, and `SubscriptionSweepService` sets it automatically ("this sweep never takes the product away from anyone"). While it was missing from `ENTITLED_STATUSES`, the onboarding gate contradicted both and one late renewal webhook locked a paying customer out. Only `expired` and `cancelled` reach the locked screen.
- ⚠️ **Trial expiry is announced before it happens.** `resolveBillingNotice` (`lib/billing/notice.ts`, pinned) drives a strip above the dashboard header in the last 5 days, and for `past_due` it says the workspace still works. Dismissal is keyed by date, so it returns tomorrow — a deadline banner that dismisses for ever is decoration. It reads the gate's existing `GET /api/onboarding` through `BillingStandingProvider`, never `useAuth().subscription` (that is the *current user's* row, so teammates of a paid workspace would see nothing).
- ⚠️⚠️ **`subscription/razorpay/confirm-payment` had three defects and all three are fixed — do not reintroduce them.** (1) It **verified no HMAC signature**, so "I paid for GROWTH" was a claim any signed-in session could POST; it now checks `<order_id>|<payment_id>` with `timingSafeEqual`, like `POST /ai/credits/verify` always has. (2) It **granted a fresh trial on payment** — `status = 'trial'` plus a new 15-day window whenever the plan had `trial_days`, which every selectable plan does — so paying ₹300 bought a trial, walked through 074's latch, and left a paying customer invisible to MRR. Payment now lands `active`, period starting today, **trial columns untouched** (they are the record of the trial they already had). (3) It wrote the **caller's** row, so an admin's payment created a second subscription nothing reads while the workspace stayed locked; both routes are now `RequireRole('owner')` and the write resolves `accounts.owner_user_id` itself. Pinned by `razorpay.controller.test.ts`. Checkout lives in ONE place, `useRazorpayCheckout` — `/pricing`'s private copy is what forgot to send the signature.
- ⚠️⚠️ **THE PAYMENT WEBHOOKS HAD THE SAME TRIAL MATHS, AND IT OUTRANKED THE BROWSER CALLBACK.** All three payment-success handlers in `subscription-webhooks.controller.ts` (Razorpay `payment.captured`/`order.paid`, Razorpay `subscription.activated`, Stripe `checkout.session.completed`) computed status as "grant a trial, else carry the old window forward, else `expired`" — correct for a plan CHANGE, catastrophic for a PAYMENT. A lapsed customer paying to come back was set straight back to `expired` **by their own successful payment**, and because the webhook lands after `confirm-payment` it overwrote the `active` that had just let them in: pay, get in, get thrown out. **Money arrived ⇒ `active`**, period starting now, trial columns and the 074 latch untouched. Pinned by `subscription-webhooks.controller.test.ts`.
