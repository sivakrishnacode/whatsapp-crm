...
User-visible changes in `conceps-wa`. Self-hosters: when pulling an update,
...
Assistant**; Conceps WA calls the provider directly with that key, so
there's no per-seat AI fee and your conversation data never leaves
your own infrastructure for a Conceps WA-run service. The key is stored
...
- **Outbound event webhooks (`/api/v1/webhooks`).** Register an HTTPS
  ...
  Each delivery is signed with an `X-Conceps-WA-Signature`
...
Multi-user accounts ship. Every Conceps WA install is multi-tenant on the
...
- **Public REST API (`/api/v1`) — groundwork.** A scoped, revocable
  **API key** system so you can drive Conceps WA from your own scripts and
...
- **Configurable default deal currency.** Each account can now pick
  ...
  [Default currency](https://conceps.tech/docs/settings#deals).
- **Members tab in Settings.** The user-facing surface for the
  ...
  [Members docs](https://conceps.tech/docs/members).
...
Conceps WA is intentionally **single-tenant per WhatsApp number**. RLS on
