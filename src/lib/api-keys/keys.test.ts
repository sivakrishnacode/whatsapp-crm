...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('conceps_live_abc')).toBe(hashApiKey('conceps_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('conceps_live_abc')).not.toBe(hashApiKey('conceps_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('conceps_live_xyz');
