...
export async function sendTemplateMessage(
  ...
  if (!response.ok) {
    const metaErr = await response.json().catch(() => null)
    const errMsg = metaErr?.error?.message ?? `Meta API error: ${response.status}`
    if (metaErr?.error?.code === 132000 || /132000/.test(errMsg)) {
      console.error(
        '[meta-api] Meta parameter count mismatch. Sent payload:',
        rawBody,
        '| Meta error:',
        errMsg,
      )
