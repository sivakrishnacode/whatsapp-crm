...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Conceps WA account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Conceps WA account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our Conceps WA account';
    const message = `Join ${accountName} on Conceps WA using this link (valid for ${result?.expiresInDays} days): ${url}`;
          ...
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              {/* Anchor styled with `buttonVariants` rather than wrapping
                  in <Button asChild>. The Conceps WA Button is the Base UI
