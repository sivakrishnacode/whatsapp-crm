...
export async function GET() {
  try {
    ...
    const { data: config, error: configError } = await supabase
      ...
      .select('phone_number_id, access_token, status, token_expires_at')
    ...
    try {
      ...
      // Embedded Signup connections carry a long-lived USER token
      // (~60 days), not a non-expiring System User token — flag it a
      // week out so the UI can prompt a re-auth before it silently
      // stops working. Manual rows have no token_expires_at and never
      // trigger this (the operator owns rotation for those).
      const SEVEN_DAYS_MS = 7 * 24 * 60 * 60 * 1000
      const tokenExpiresAt = config.token_expires_at as string | null
      const tokenExpiringSoon = Boolean(
        tokenExpiresAt && new Date(tokenExpiresAt).getTime() - Date.now() < SEVEN_DAYS_MS
      )
      return NextResponse.json({
        connected: true,
