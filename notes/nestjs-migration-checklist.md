...
- **[FIXED] `next.config.ts` rewrites cleanup** — dead `if (!nestApiUrl)` branch removed, `require("fs")` → top-level import. **Gotcha confirmed the hard way: the Docker branch must take precedence over `NEST_API_URL`.** Next.js resolves `rewrites()` at *build* time into `routes-manifest.json`, and the image build copies `.env.local` (needed for `NEXT_PUBLIC_*` inlining) whose `NEST_API_URL=http://localhost:8001` would get baked in under env-first ordering → every proxied `/api/*` call ECONNREFUSEDs inside the container. Docker-first (`DOCKERIZED=true` is set in the Dockerfile, so builds take that branch) bakes `http://api:8001` correctly.
